...
- **Fase V25 (Completa)**: Sometimiento a validación del empaquetado final en Mozilla Add-ons (AMO). Se solventó el error crítico de permisos inyectando correctamente el objeto `{"required": ["none"]}` en `data_collection_permissions` del manifest. Se proveyó el código fuente crudo (ZIP de TypeScript/Vite) para satisfacer los requisitos de revisión humana de ofuscación de Mozilla. Extensión a la espera de firma y empaquetado criptográfico.
